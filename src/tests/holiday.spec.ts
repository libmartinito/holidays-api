...
let authRoute: AuthRoute, holidayRoute: HolidayRoute, app: App, users: any, holidays: any, authToken: string
...
//   app = new App([authRoute, holidayRoute])
  ...
  // authToken = authRes.header['set-cookie'][0].split(';')[0].split('Authorization=')[1]
// })
...
describe('Holiday', () => {
  ...
  describe('[POST] /save-holiday', () => {
    test('should return saved holiday when request is authenticated and valid', async () => {
      ...
      const holidayRes = await request(app.getServer()).post(`${holidayRoute.path}save-holiday`).set({ 'authorization': 'Bearer ' + authToken }).send(holidayData)
  ...
  describe('[DELETE] /unsave-holiday', () => {
    test('should return delete successful message when authenticated and request is valid', async () => {
      ...
      const authRes = await request(app.getServer()).post(`${authRoute.path}login`).send(userData)
      const authToken = authRes.header['set-cookie'][0].split(';')[0].split('Authorization=')[1]
      const holidayRes = await request(app.getServer()).delete(`${holidayRoute.path}unsave-holiday`).set({ 'authorization': 'Bearer ' + authToken }).send(holidayData)
